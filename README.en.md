...
- Current specifications support 8–90 seconds, 1920×1080 landscape or 1080×1920 portrait, at 30 fps. Duration follows approved copy and measured narration.
...
The CLI uses the positional `help` command and does not support `--help`. Direct `render` accepts `draft` or `high`; For current-policy tasks, `run` defaults to draft; explicit `--quality high` also requires actual human visual and voice approval bound to the current plan. Historical tasks without a generator policy retain their draft-then-high sequence. Capture uses repository-local HyperFrames capture and falls back to supplied assets after a capture failure. `--supplied-only` skips URL capture.
...
After full copy approval, an explicit `voiceProfile` selects the configured local Kokoro or Qwen reference-voice route. Valid approved recordings are reused without silently changing voices or restarting auditions. Qwen uses measured Whisper DTW alignment; Kokoro uses provider timing. Both produce WAV, transparent in-frame captions and SRT. See the [local Chinese narration guide](docs/guides/CHINESE-TTS.md) for setup and private configuration prerequisites. Its retained benchmark commands require the corresponding private local benchmark data and authorized assets; they are not generic fresh-clone examples.
...
The transitive `adm-zip` dependency is locked to `0.6.1`, which patches extraction through destination links; HyperFrames remains at `0.8.33`. `check:dependency-risk` verifies the reviewed patch and code, but does not replace `npm audit` before each release. Read the [dependency risk disposition](docs/security/DEPENDENCY-RISK.md) for validation and limits.
